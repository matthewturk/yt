...
cdef int walk_volume_spherical(VolumeContainer *vc,
    ...
    for i in range(3):
        min_bound[i] = vc.left_edge[i]
        max_bound[i] = vc.right_edge[i]

    num_radial_voxels = vc.dims[0]
    num_polar_voxels = vc.dims[1]
    num_azimuthal_voxels = vc.dims[2]
    ...
    for i in range(voxels.size()):
        ...
        cur_ind[1] = voxels[i].polar
        cur_ind[2] = voxels[i].azimuthal
